...
% [3] https://github.com/casadi/casadi/discussions/3337
    ...
    if(mode == 1) % classic nlpsol s_function
        ...
        s_func_name_new          = ['s_function_', casadi_fun_c_header_name]; % final name for Simulink s-function
        s_fun_c_file_path        = [output_dir, s_func_name_new];

        copyfile(s_func_name_origin, s_fun_c_file_path, 'f');
        replace_strings_in_casadi_file(s_fun_c_file_path, s_func_name_new);
        ...
        mex(['-I' inc_path],['-L' lib_path],'-lcasadi', s_fun_c_file_path, '-output', output_dir);
        %file_name = 'f_opt.casadi';
        
        delete(s_fun_c_file_path);
    elseif(mode == 2) % modified combination of [2] and [3]
        s_func_name_origin = [output_dir, s_fun_name];
        s_func_name_new = [output_dir, 's_function_', casadi_fun_c_header_name];
        ...
        % for later use of 'fullsimu' mode
        copyfile([output_dir, 's_function_', casadi_fun_name, '.mexa64'], [output_dir, casadi_fun_name, '_matlab.mexa64'], 'f');
    else
