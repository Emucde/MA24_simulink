...
int main(int argc, char **argv) {
    ...
    // Combine qq1 and qq2 into qq_all
    double **qq_all = (double **)malloc(rows * sizeof(double *));
    for (int i = 0; i < rows; i++) {
        qq_all[i] = (double *)malloc(cols / 2 * sizeof(double));
        memcpy(qq_all[i], &data[i][i < rows / 2 ? 0 : cols / 2], cols / 2 * sizeof(double));
    }

    // Sorting poses based on distances
    double **sorted_pose = (double **)malloc(rows * sizeof(double *));
    for (int i = 0; i < rows; i++) {
        sorted_pose[i] = (double *)malloc(cols / 2 * sizeof(double));
        memset(sorted_pose[i], 0, cols / 2 * sizeof(double));
    }

    double pose_distances[rows];
    
    memcpy(sorted_pose[0], qq_all[0], cols / 2 * sizeof(double)); // Initialize sorted_pose with the first pose
    ...
    for (int i = 0; i < rows - 1; i++) { // Iterate over all original rows
        ...
        for (int j = 1; j < remaining_rows; j++) { // Find closest pose in remaining poses
            double distance = calculate_norm(sorted_pose[i], qq_all[j], cols / 2);
        ...
        memcpy(sorted_pose[i + 1], qq_all[min_idx], cols / 2 * sizeof(double));

        // Remove min_idx from qq_all by overwriting it with the last element
        memcpy(qq_all[min_idx], qq_all[remaining_rows - 1], cols / 2 * sizeof(double));
    ...
    write_csv(output_file, sorted_pose, rows, cols / 2);
    ...
    for (int i = 0; i < rows; i++) free(qq_all[i]);
    free(qq_all);
