#define S_FUNCTION_NAME  s_function_opti_robot_model_bus_fun
#define S_FUNCTION_LEVEL 2

#include "simstruc.h"
#include "robot_model_bus_fun.h"
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    int_T n_in  = robot_model_bus_fun_n_in();
    int_T n_out = robot_model_bus_fun_n_out();
    int_T sz_arg, sz_res, sz_iw, sz_w;
    robot_model_bus_fun_work(&sz_arg, &sz_res, &sz_iw, &sz_w);
    
    /* Set up simulink input/output ports */
    int_T i;
    if (!ssSetNumInputPorts(S, n_in)) return;
    for (i=0;i<n_in;++i) {
      const int_T* sp = robot_model_bus_fun_sparsity_in(i);
    ...
    for (i=0;i<n_out;++i) {
      const int_T* sp = robot_model_bus_fun_sparsity_out(i);
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    */
    ...
    int mem = robot_model_bus_fun_checkout();

    /* Run the CasADi function */
    robot_model_bus_fun(arg,res,iw,w,mem);

    /* Release hold */
    robot_model_bus_fun_release(mem);
}

static void mdlTerminate(SimStruct *S) {
  /* Signal that we no longer want to use the CasADi Function */
  robot_model_bus_fun_decref();
}
