...
{
  {
    ...
    static const int16_T b_1[7] = { 1000, 1000, 1000, 1000, 50, 50, 50 };

    static const int16_T c_0[7] = { 100, 200, 500, 200, 50, 50, 10 };

    static const int16_T d[7] = { 200, 400, 1000, 400, 100, 100, 20 };

    static const int8_T d_0[6] = { 0, 1, 3, 4, 5, 6 };
    ...
    if (realtime_simu_franka_fr3_DW.jointspacectlsubsys_MODE) {
      ...
      } else if (absxk - realtime_simu_franka_fr3_DW.t_start <
                 realtime_simu_franka_fr3_P.t1_Value) {
        for (c = 0; c < 7; c++) {
          realtime_simu_franka_fr3_B.K_d[c] = c_0[c];
          realtime_simu_franka_fr3_B.D_d[c] = d[c];
          realtime_simu_franka_fr3_B.D_d[c] = sqrt
            (realtime_simu_franka_fr3_B.D_d[c]);
        }
      } else {
        for (k = 0; k < 7; k++) {
          realtime_simu_franka_fr3_B.K_d[k] = b_1[k];
          realtime_simu_franka_fr3_B.D_d[k] = d[k];
          realtime_simu_franka_fr3_B.D_d[k] = sqrt
            (realtime_simu_franka_fr3_B.D_d[k]);
        }
      }
    ...
    if (rtb_reset > 0.0) {
      ...
      q4 = R[6] - R[2];
      g_a = R[1] - R[3];
      rtb_reset = sqrt(((rtb_reset * rtb_reset + q4 * q4) + g_a * g_a) / (((3.0
        - R[0]) - R[4]) - R[8])) / 2.0;
    }
    ...
    if (s4 > 0.0) {
      ...
      q4 = R[2] + R[6];
      g_a = R[5] + R[7];
      q4 = sqrt(((s4 * s4 + q4 * q4) + g_a * g_a) / (((R[0] + 3.0) + R[4]) - R[8]))
        / 2.0;
    }

    s4 = (static_cast<real_T>(R[5] - R[7] >= 0.0) * 2.0 - 1.0) * s2;
    ...
    realtime_simu_franka_fr3_B.q_e_o[2] = (time_end * s2 + rtb_reset * t) + (s *
      s3 - s4 * absxk);
