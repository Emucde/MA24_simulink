#define S_FUNCTION_NAME  s_function_opti_MPC8
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    int_T n_in  = MPC8_n_in();
    int_T n_out = MPC8_n_out();
    int_T sz_arg, sz_res, sz_iw, sz_w;
    MPC8_work(&sz_arg, &sz_res, &sz_iw, &sz_w);
    
    /* Set up simulink input/output ports */
    int_T i;
    if (!ssSetNumInputPorts(S, n_in)) return;
    for (i=0;i<n_in;++i) {
      const int_T* sp = MPC8_sparsity_in(i);
    ...
    for (i=0;i<n_out;++i) {
      const int_T* sp = MPC8_sparsity_out(i);
...
static void mdlStart(SimStruct *S) {
    // Annahme: Die Datentyp-ID wird als Parameter übergeben
    DTypeId solver_stats_bus_id = (DTypeId)mxGetScalar(ssGetSFcnParam(S, 0));
}
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    */
    ...
    int mem = MPC8_checkout();

    /* Run the CasADi function */
    MPC8(arg,res,iw,w,mem);
