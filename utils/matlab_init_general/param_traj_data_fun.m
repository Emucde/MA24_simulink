function param_traj_data_out = param_traj_data_fun(traj_settings, method, data_index, param_traj_data_in, set_data)
    ...
    if(strcmp(method, 'init'))
        ...
        for i = 1:numel(fields)
            ...
            if(size(val, 2) == 1)
                bus_temp_struct.(fields{i}) = repmat(val, 1, traj_settings.N_data, traj_settings.traj_amount);
            else
                bus_temp_struct.(fields{i}) = repmat(val, 1, 1, traj_settings.N_data, traj_settings.traj_amount);
            end
        end
