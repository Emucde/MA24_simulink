...
param_weight.(MPC).R_q_ref      = 1e-10*diag(ones(n,1));
param_weight.(MPC).R_q_p        = 1e-5*diag(ones(n,1));
...
param_weight.(MPC).epsilon_t = 1e-5;%inf;
param_weight.(MPC).epsilon_r = 1e-5;%inf;
...
param_weight.(MPC).R_q_ref   = 1e0*diag(ones(n,1));
...
% param_weight.(MPC).R_q_p    = 1e-2*diag(ones(n, 1));
% param_weight.(MPC).R_q_pp   = 1e-5*diag(ones(n, 1));
% param_weight.(MPC).R_x_prev  = 1e-2*diag([1*ones(n,1); 0*ones(n,1)]);
...
param_weight.(MPC).lambda_u  = 500*ones(n,1);
...
MPC='MPC11';
param_weight.(MPC).Q_y    = 1e2*diag([1*ones(3,1); 1e-3*ones(3,1)]);  % d_kpn
param_weight.(MPC).Q_yN   = 1e5*diag([1*ones(3,1); 1e-3*ones(3,1)]);  % D_N
...
param_weight.(MPC).R_q_ref = 1e-0*diag(ones(n,1));
param_weight.(MPC).R_q_p   = 1e-2*diag(ones(n,1));  % d_kpn
param_weight.(MPC).R_q_pp  = 1e-5*diag(ones(n,1));
