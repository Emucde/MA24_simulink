...
%%%%%%%%%%%%% PFUSCH!!! TODO
try
    yyN_ineq_eps = eval("param_weight_"+casadi_func_name+".yN_ineq_eps;"); % >= 0 (if 0 then hard constrained)
catch
    yyN_ineq_eps = -1;
end
try
...
if(weights_and_limits_as_parameter)
    Q =        MX.sym('Q', 2,2); % x y weight
    R_u =      MX.sym('R_u', 2,2); % u absolut
    R_du =     MX.sym('R_du', 2,2); %un-un-1
    R_dx =     MX.sym('R_dx', 4,4); % x = d/dt q, d^2/dt^2 q
    R_dx_km1 = MX.sym('R_dx_km1', 4,4); % xk-xkm1
    x_min =    MX.sym('x_min', 4,1);    
    x_max =    MX.sym('x_max', 4,1);
    u_min =    MX.sym('u_min', 2,1);
    u_max =    MX.sym('u_max', 2,1);
    if(terminal_ineq_yref_N)
        yN_ineq_eps = MX.sym('yN_ineq_eps', 1,1);
    end
    if(terminal_soft_yref_N)
        Q_yN_soft   = MX.sym('Q_yN_soft', 2,2);
    end
...
% needed:
x       = MX.sym('x',      2*n, N_MPC+1 );
u       = MX.sym('u',      n,   N_MPC   );

% parameter
x_k       = MX.sym('x_k',    2*n, 1      );
u_km1     = MX.sym('us_km1', n,   1      );
y_kp1_ref = MX.sym('ys_ref', 2,   N_MPC  );
...
dx = MX(2*n,N_MPC); % = [d/dt q, d^2/dt^2 q] = d/dt x 
...
for i=1:N_MPC
    ...
    % calculate output
    H_e = hom_transform_endeffector_casadi(x(1:n,i+1), param_robot);
    y(:,i) = H_e(1:2,4);
end
...
get_prob = Function('prop_fun', {y_kp1_ref, x_k, u_km1, x, u, Q, R_u, R_du, R_dx, R_dx_km1, x_min, x_max, u_min, u_max, Q_yN_soft}, {J, vertcat(w{:}), vertcat(g{:}), vertcat(p{:})});
get_prob_SX = get_prob.expand();

[JT, WT, GT, PT] = get_prob_SX(y_kp1_ref, x_k, u_km1, x, u, Q, R_u, R_du, R_dx, R_dx_km1, x_min, x_max, u_min, u_max, Q_yN_soft);

prob = struct('f', JT, 'x', WT, 'g', GT, 'p', PT);
...
sol_sym = solver('x0', [x(:); u(:)], 'lbx', lbw, 'ubx', ubw,...
            'lbg', lbg, 'ubg', ubg, 'p', p);
...
if(weights_and_limits_as_parameter)
    if(terminal_ineq_yref_N)
        f_opt = Function(casadi_func_name, ...
            {y_kp1_ref, x_k, u_km1, x, u, Q, R_u, R_du, R_dx, R_dx_km1, x_min, x_max, u_min, u_max, y_Nineq_eps},...
            {u_full_opt(:,1), x_full_opt, u_full_opt, J_y, J_u, J_ukm1, J_dx, J_dxkm1},...
            {'yref', 'xk', 'ukm1', 'x_init_guess', 'u_init_guess', 'Q', 'R_u', 'R_du', 'R_dx', 'R_dx_km1', 'x_min', 'x_max', 'u_min', 'u_max', 'yNineq_eps'},...
            {'u_opt', 'x_full_opt', 'u_full_opt', 'Jy', 'Ju', 'Jukm1', 'Jdx', 'Jdxkm1'});
        
        [u_opt_sol, x_full_opt_sol, u_full_opt_sol, Jy_sol, Ju_sol, Jukm1_sol, Jdx_sol, Jdxkm1_sol] = f_opt(y_ref_0, x_0_0, u_init_guess_0(:,1), x_init_guess_0, u_init_guess_0, QQ, RR_u, RR_du, RR_dx, RR_dx_km1, xx_min, xx_max, uu_min, uu_max, yyN_ineq_eps);
    elseif(terminal_soft_yref_N)
        f_opt = Function(casadi_func_name, ...
            {y_kp1_ref, x_k, u_km1, x, u, Q, R_u, R_du, R_dx, R_dx_km1, x_min, x_max, u_min, u_max, Q_yN_soft},...
            {u_full_opt(:,1), x_full_opt, u_full_opt, J_y, J_u, J_ukm1, J_dx, J_dxkm1},...
            {'yref', 'xk', 'ukm1', 'x_init_guess', 'u_init_guess', 'Q', 'R_u', 'R_du', 'R_dx', 'R_dx_km1', 'x_min', 'x_max', 'u_min', 'u_max', 'Q_yN_soft'},...
            {'u_opt', 'x_full_opt', 'u_full_opt', 'Jy', 'Ju', 'Jukm1', 'Jdx', 'Jdxkm1'});
...
us         = MX.sym('us'  ,      size(u         ));
xs         = MX.sym('xs'  ,      size(x         ));
Qs         = MX.sym('Qs',        size(Q         )); % TODOO!!!! if fehlt
...
if(weights_and_limits_as_parameter)
    ...
    if(terminal_ineq_yref_N)
        yNs_ineq_eps = MX.sym('yNs_ineq_eps', 1,1);
        ubg = [zeros(numel(x), 1); yNs_ineq_eps];
        pp = [pp; yNs_ineq_eps];
    end
    %else % TODO!!!!
    %    yNs_ineq_eps = yN_ineq_eps;
    %end
    if(terminal_soft_yref_N)
        Q_yNs_soft   = MX.sym('Q_yNs_soft', 2,2);
        pp = [pp; Q_yNs_soft(:)];
    end
end
...
if(terminal_soft_yref_N)
    JyNs = dot(ys_kp1(:,end)-ys_kp1_ref(:,end), mtimes(Q_yNs_soft, ys_kp1(:,end)-ys_kp1_ref(:,end)));
    %JyNs = norm_2(ys_kp1(:,end)-ys_kp1_ref(:,end));
else
    JyNs = 0;
end
...
Js = Jsy + Jsu + Jsukm1 + Jsdx + Jsdxkm1 + JyNs;
...
if(weights_and_limits_as_parameter)
    if(terminal_ineq_yref_N)
        f_opt = Function(casadi_func_name, ...
            {ys_kp1_ref, xs_k, us_km1, xs, us, Qs, Rs_u, Rs_du, Rs_dx, Rs_dx_km1, xs_min, xs_max, us_min, us_max, yNs_ineq_eps},...
            {u_full_opt(:,1), x_full_opt, u_full_opt, Jsy, Jsu, Jsukm1, Jsdx, Jsdxkm1},...
            {'yref', 'xk', 'ukm1', 'x_init_guess', 'u_init_guess', 'Q', 'R_u', 'R_du', 'R_dx', 'R_dx_km1', 'x_min', 'x_max', 'u_min', 'u_max', 'yNs_ineq_eps'},...
            {'u_opt', 'x_full_opt', 'u_full_opt', 'Jy', 'Ju', 'Jukm1', 'Jdx', 'Jdxkm1'});
        
        [u_opt_sol, x_full_opt_sol, u_full_opt_sol, Jy_sol, Ju_sol, Jukm1_sol, Jdx_sol, Jdxkm1_sol] = f_opt(y_ref_0, x_0_0, u_init_guess_0(:,1), x_init_guess_0, u_init_guess_0, QQ, RR_u, RR_du, RR_dx, RR_dx_km1, xx_min, xx_max, uu_min, uu_max, yyN_ineq_eps);
    elseif(terminal_soft_yref_N)
        f_opt = Function(casadi_func_name, ...
            {ys_kp1_ref, xs_k, us_km1, xs, us, Qs, Rs_u, Rs_du, Rs_dx, Rs_dx_km1, xs_min, xs_max, us_min, us_max, Q_yNs_soft},...
            {u_full_opt(:,1), x_full_opt, u_full_opt, Jsy, Jsu, Jsukm1, Jsdx, Jsdxkm1},...
            {'yref', 'xk', 'ukm1', 'x_init_guess', 'u_init_guess', 'Q', 'R_u', 'R_du', 'R_dx', 'R_dx_km1', 'x_min', 'x_max', 'u_min', 'u_max', 'Q_yN_soft'},...
            {'u_opt', 'x_full_opt', 'u_full_opt', 'Jy', 'Ju', 'Jukm1', 'Jdx', 'Jdxkm1'});
