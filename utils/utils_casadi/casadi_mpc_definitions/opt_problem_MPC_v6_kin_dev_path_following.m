...
% p_d_0 = repmat(param_trajectory.p_d( 1:3, 1 ), 1, numel(MPC_traj_indices)); % (y_0 ... y_N)
% q_d_0 = repmat(param_trajectory.q_d( 1:4, 1 ), 1, numel(MPC_traj_indices)); % (q_0 ... q_N)
y_d_0 = [p_d_0; q_d_0];
...
if(implicit_xk)
    lam_g_init_guess_0 = zeros(numel(u_init_guess_0) + numel(x_init_guess_0(1+n_red:2*n_red, 2:end)) + numel(x_0_0) + numel(theta_0), 1);
else
    lam_g_init_guess_0 = zeros(numel(u_init_guess_0) + numel(x_init_guess_0(1+n_red:2*n_red, :)) + numel(x_0_0) + numel(theta_0), 1);
end
...
J_q_prev = Q_norm_square(x(1:n_red, :) - q_prev, pp.R_q_prev(n_indices, n_indices)); %Q_norm_square(u, pp.R_u);
...
cost_vars_names = '{J_yt, J_yt_N, J_yr, J_yr_N, J_q_p, J_q_pp, J_q_prev, J_theta, J_thetaN, J_theta_prev}';
