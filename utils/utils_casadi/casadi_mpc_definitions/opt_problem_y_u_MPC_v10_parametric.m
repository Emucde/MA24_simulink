...
for i=0:N_MPC
    ...
    if(i < N_MPC)
        ...
        %     g_x(1, 1 + (i+1))  = { F_kp1(  x_k_i, tau_i ) - x( :, 1 + (i+1)) }; % Set the state constraints for xk = x(t0) = tilde x0 to xk+1 = x(t0+Ta)
        % elseif(i == 1)
        %     g_x(1, 1 + (i+1))  = { F2(  x_k_i, tau_i ) - x( :, 1 + (i+1)) }; % Set the state constraints for xk+1 = x(t0+Ta) to x(t0+Ts_MPC) = tilde x1
        % else
