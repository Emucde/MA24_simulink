function q = rotm2quat_v3_casadi(R)
    ...
    %my_sign = @(x) 2*(x >= 0) - 1;
    x = SX.sym('x', 1, 1);
    mysign = Function('mysign', {x}, {if_else(x >= 0, 1, -1)});
    ...
    q1 = mysign(r32 - r23) * sqrt(abs(r11 - r22 - r33 + 1)) / 2;
    q2 = mysign(r13 - r31) * sqrt(abs(r22 - r33 - r11 + 1)) / 2;
    q3 = mysign(r21 - r12) * sqrt(abs(r33 - r11 - r22 + 1)) / 2;
    
    % Assemble the quaternion
    q = [q0; q1; q2; q3];
    %warning('rotm2quat_v3_casadi is not robust to numerical errors for numbers < 2.2204e-16 = eps. Use rotation2quaternion instead.');
