...
if(compile_sfun)
    if(compile_mode == 1)
        tic;
        compile_casadi_sfunction(f_opt, [s_fun_path, '/'], output_dir, MPC_solver, '-O3', compile_mode, remove_sourcefiles, use_jit); % default nlpsol s-function
        disp(['Compile time for casadi s-function (nlpsol): ', num2str(toc), ' s']);
    elseif(compile_mode == 2)
        tic;
        compile_casadi_sfunction(f_opt, [s_fun_path, '/'], output_dir, MPC_solver, '-O3', compile_mode, remove_sourcefiles, use_jit); % default opti s-function
