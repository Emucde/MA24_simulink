function closeAllSimulinkModels(folderPath, excludedFileName)
  % closes all Simulink models with the .slx extension in a given folder and its subfolders
  % excludes a specific Simulink file from being closed if provided
  
  arguments
    folderPath (1,1) string
    excludedFileName (1,1) string = ''
  end
  ...
  for item = contents'
    ...
    if item.isdir && ~strcmp(item.name, '.') && ~strcmp(item.name, '..')
      ...
      closeAllSimulinkModels(fullfile(folderPath, item.name), excludedFileName);
    elseif endsWith(item.name, '.slx')
      % Check if an excludedFileName is provided
      if isempty(excludedFileName) || ~contains(item.name, excludedFileName)
