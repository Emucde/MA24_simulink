#define S_FUNCTION_NAME  s_function_MPC1
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    if (!mxIsChar(ssGetSFcnParam(S, 0))) {
      mexErrMsgIdAndTxt( "MATLAB:s_function_MPC1:invalidParameter",
    ...
    if (ret) {
      mexErrMsgIdAndTxt( "MATLAB:s_function_MPC1:Load",
                         "Failed to load file.");
    }
    mexPrintf("success\n");
    ...
    for (i=0;i<n_in;++i) {
      ...
      if (nnz!=sp[0]*sp[1]) {
        casadi_c_pop();
        mexErrMsgIdAndTxt( "MATLAB:s_function_MPC1:sparsity",
