#define S_FUNCTION_NAME  s_function_opti_MPC01
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    int_T n_in  = MPC01_n_in();
    int_T n_out = MPC01_n_out();
    int_T sz_arg, sz_res, sz_iw, sz_w;
    MPC01_work(&sz_arg, &sz_res, &sz_iw, &sz_w);
    
    /* Set up simulink input/output ports */
    int_T i;
    if (!ssSetNumInputPorts(S, n_in)) return;
    for (i=0;i<n_in;++i) {
      const int_T* sp = MPC01_sparsity_in(i);
    ...
    for (i=0;i<n_out;++i) {
      const int_T* sp = MPC01_sparsity_out(i);
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    */
    ...
    int mem = MPC01_checkout();

    /* Run the CasADi function */
    MPC01(arg,res,iw,w,mem);
