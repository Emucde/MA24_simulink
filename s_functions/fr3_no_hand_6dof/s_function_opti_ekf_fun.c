#define S_FUNCTION_NAME  s_function_opti_ekf_fun
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    int_T n_in  = ekf_fun_n_in();
    int_T n_out = ekf_fun_n_out();
    int_T sz_arg, sz_res, sz_iw, sz_w;
    ekf_fun_work(&sz_arg, &sz_res, &sz_iw, &sz_w);
    
    /* Set up simulink input/output ports */
    int_T i;
    if (!ssSetNumInputPorts(S, n_in)) return;
    for (i=0;i<n_in;++i) {
      const int_T* sp = ekf_fun_sparsity_in(i);
    ...
    for (i=0;i<n_out;++i) {
      const int_T* sp = ekf_fun_sparsity_out(i);
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    */
    ...
    int mem = ekf_fun_checkout();

    /* Run the CasADi function */
    ekf_fun(arg,res,iw,w,mem);
