...
sem_t* open_write_sem(const char *name, rclcpp::Logger logger)
{
    ...
    if (sem == SEM_FAILED)
    {
        RCLCPP_ERROR(logger, "open_write_sem: Error opening semaphore: %s", strerror(errno));
...
int open_read_shm(const char *name, rclcpp::Logger logger)
{
    ...
    if (fd == -1)
    {
        RCLCPP_ERROR(logger, "open_write_shm: Error opening shared memory: %s", strerror(errno));
...
int read_shared_memory(int fd, double *data, size_t size, rclcpp::Logger logger)
{
    ...
    {
        RCLCPP_ERROR(logger, "read_shared_memory: Error mapping shared memory: %s", strerror(errno));
        return -1;
    }
    ...
    {
        RCLCPP_ERROR(logger, "read_shared_memory: Error unmapping shared memory: %s", strerror(errno));
        return -1;
    }
...
int read_shared_memory_flag(int fd, int8_t *data, size_t size, rclcpp::Logger logger)
{
    ...
    if (shared_data == MAP_FAILED)
    {
        RCLCPP_ERROR(logger, "read_shared_memory_flag: Error mapping shared memory: %s", strerror(errno));
        return -1;
    }
    ...
    if (munmap(shared_data, size) == -1)
    {
        RCLCPP_ERROR(logger, "read_shared_memory_flag: Error unmapping shared memory: %s", strerror(errno));
        return -1;
    }
...
int write_to_shared_memory(int fd, const void *data, size_t size, rclcpp::Logger logger)
{
    ...
    if (shared_data == MAP_FAILED)
    {
        RCLCPP_ERROR(logger, "write_to_shared_memory: Error mapping shared memory: %s", strerror(errno));
        return -1;
    }
    ...
    if (munmap(shared_data, size) == -1)
    {
        RCLCPP_ERROR(logger, "write_to_shared_memory: Error unmapping shared memory: %s", strerror(errno));
        return -1;
    }
