...
namespace franka_example_controllers
{
      ...
      const rclcpp::Duration & /*period*/)
  {
    ...
    if (mpc_started)
    {
      ...
      if (tau_full_future_started)
      {
        if (tau_full_future_valid)
        {
          ...
          if (tau_full_future.wait_for(std::chrono::milliseconds(0)) == std::future_status::ready)
          {
            ...
            tau_full_future_valid = false; // Indicate the future has been consumed
          }
          else
          {
            ...
            {
              // Output warning if invalid counter exceeds the maximum
              RCLCPP_WARN(get_node()->get_logger(), "No valid Python data (%d/%d). Using previous torques.", invalid_counter, MAX_INVALID_COUNT);
              invalid_counter = 0; // Reset the counter for the next cycle
            }
            // Use the previous value of tau_full if the data is not ready
          }
        }
      }
      ...
      if (!tau_full_future_started)
      {
        tau_full_future_valid = true;

        // Launch the MPC computation asynchronously
        // tau_full_future = std::async(std::launch::async, [this, state]()
        //                              {
        //                               tau_full = controller.solveMPC(state);
        //                               tau_full_future_valid = true;
    ...
    // calculate torques for next update
    if (mpc_started)
    {
      tau_full = controller.solveMPC(state);
    }
