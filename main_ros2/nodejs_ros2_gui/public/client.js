...
function start() {
    ...
    try {
        ...
        ws_data_logger = new WebSocket('ws://localhost:8081');
    }
    catch (e) {
        console.log("error ", e);
    }

    ws_data_logger.onopen = function (e) {
        console.log("Data Logger connected");
    }

    ws_data_logger.onclose = function () {
        console.log("Data Logger disconnected");
    }

    ws_data_logger.onmessage = async (event) => {
        var data = event.data;
        // check if data is a bytearray
        if (data instanceof Blob) {
            var floatValues = await receiveFloatArray(event.data);
            // console.log('Received float:', floatValues);
            updateGraph(floatValues);
        }
    }
...
function updateGraph(tau_values) {
    // Remove the first row (shift left)
    plotdata.shift();

    // Add new data point at the end with correct x value
    const newX = plotdata.length; // Next x value based on length
    plotdata.push([newX, ...tau_values]);

    // Fix x values to maintain sequential order
    for (let i = 0; i < plotdata.length; i++) {
        plotdata[i][0] = i; // Ensure x values are in order
    }
